...
Every paid model node declares registry pricing, a per-request USD cap, a total token limit, and a model-specific local tokenizer mapping. The mapping selects a `tiktoken` encoding or a SHA-256-pinned Hugging Face tokenizer JSON file and supplies explicit message, tool, and safety overheads. Before dispatch, the proxy counts the complete prompt, rejects an over-cap input, clamps the output token maximum to the remaining context and USD cap, and reserves that maximum against the same spend ledger used by tools. Provider-reported usage reconciles the reservation; a node is quarantined if its reported usage exceeds the registered bound. The financial cap's accuracy depends on the operator verifying that tokenizer and overhead mapping against the upstream provider's billing rules. A missing or unverifiable mapping blocks paid inference.
