...
Registered remote nodes publish a same-origin health URL. The daemon polls each through its node-specific mTLS bundle and admits it for placement only when the report confirms a healthy model and available slots. Health and capacity are visible in the operator workspace. Provider discovery and rented-node lifecycle use a separate provider adapter contract.
