...
The default for `secret_sensitive` tasks remains an air-gapped local inference node. An operator may explicitly authorize individual rented or cloud node IDs after evaluating their security posture. Remote nodes are registry records with URL, model/context capabilities, allowed privacy classes, and per-node mTLS CA/client certificate/key paths. Remote inference uses mTLS with server hostname verification. Provider provisioning credentials resolve from an environment variable or operator-controlled local secret file at lifecycle-call time; they are never forwarded in prompts or inference headers. The provider-neutral lifecycle interface has a RunPod Pod reference adapter. Pod creation does not make a node eligible for inference until its approved registry record and mTLS health report pass admission.
...
Genesis creates a named `daemon_admin` with a one-time token and permanently expires the installation bootstrap token for API access. The daemon admin controls global memory and installation setup; organization admins alone may register or activate their tools and compute nodes, change provisioning settings, or modify registry shares. The `admin`, `operator`, and `viewer` roles control organization user management, task writes, and task reads; tokens are stored only as SHA-256 hashes and can be revoked. Tasks and memory claims carry an organization key. Memory authority is set per organization, defaults to `hybrid`, and combines with each task's execution profile using the stricter of `session_auto < hybrid < human_gated`. Task-scoped claims persist in the database and leave worker recall when the task ends. Organization admins may copy reviewed claims into permanent organization memory while preserving the original task record; only a daemon admin may promote `global` claims, which are visible across organizations. Previously promoted `repo:<id>` claims remain visible across tasks in the same repository. In `session_auto`, capture creates a separate untrusted organization candidate and a durable librarian job. An independent, configurable evaluator must corroborate source evidence at confidence ≥0.90 with no contradiction before the candidate becomes trusted across tasks. Worker self-scoring is never sufficient; an absent or unreachable evaluator leaves the candidate untrusted. Remote HTTPS evaluation needs both daemon-level enablement and the owning organization's admin opt-in before any organization claims leave the machine. Organizations control sharing of tool and compute registry resources with other organizations at individual-record and whole-registry scope. Sharing never transfers ownership or changes a task's own budget and privacy policy.
...
Memory claims retain numeric and labeled confidence, validity dates, tags, supersession, and source provenance. Operator recall and the control workspace include those fields and open contradiction warnings, including stale, superseded, and pending claims for audit. Worker context uses only currently valid, trusted promoted claims from the active task, its repository, its organization, or global scope, with a brief projection when requested. The SQLite and PostgreSQL schema migration preserves the original V1 table layout as revision `0001` and adds these fields in `0002` so existing databases can upgrade.

## Expanded V1 operator and infrastructure surfaces

The Control workspace supports local organization onboarding, role-scoped users and revocation, task and step management, budget controls, audit timelines, node health, and imported-tool binding review. Imported tools remain disabled while an organization admin reviews a proposed trusted built-in binding. An approved binding persists across daemon restarts; tools without a trusted built-in match remain disabled.

Cross-machine compute discovery polls configured mTLS node managers rather than accepting LAN advertisements. The provider-neutral Pod lifecycle contract has a RunPod reference adapter. Provisioning requires a price quote by default. Each organization controls its own create-then-verify-and-delete-on-over-cap fallback switch, which only its own admin user may change; the installation bootstrap token cannot override it. Every requested Pod lease must state its duration and may run for at most 24 hours. The admission gate requires a provider-confirmed termination deadline before paid creation. RunPod REST v2 has no such deadline field, and its former CLI termination timer was removed after it was found to be unenforced. Automatic paid RunPod creation is disabled in V1: `provisioning.enable_paid_runpod_leases` defaults to `false`, and a `true` value is rejected at startup while this guarantee is unavailable. Process-level watchdogs cannot establish Pod deletion or stop storage billing. The daemon should still reconcile and delete known Pods when online. RunPod's GPU catalog supplies a read-only GPU list price, while the actual Pod rate is reported after creation; storage is billed separately. The platform must not present the catalog figure as a binding total Pod price. See https://docs.runpod.io/api-reference-v2/migrate-from-v1 and https://github.com/runpod/runpodctl/pull/330.
