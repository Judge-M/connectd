...
Organization-scoped local users are bootstrapped by the existing operator token. Once a user exists, only that organization's admin may register or activate its tool and compute node resources; bootstrap cannot perform those registry writes. The `admin`, `operator`, and `viewer` roles control user management, task writes, and task reads; operator tokens are stored only as SHA-256 hashes and can be revoked. Tasks and memory claims carry an organization key so context and task timelines remain tenant-scoped. The bootstrap token can inspect and administer the installation. Memory-promotion authority is set per organization, defaults to `hybrid`, and combines with each task's execution profile using the stricter of `session_auto < hybrid < human_gated`. Task-scoped memory is linked to its task in the database; permanent organization promotion requires an organization admin in `hybrid` and `human_gated`. Organizations control sharing of tool and compute registry resources with other organizations at both individual-record and whole-registry scope; only an admin user of the owning organization can grant or revoke a share. Sharing never transfers ownership or changes a task's own budget and privacy policy.
