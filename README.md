...
Remote nodes also need a same-origin `health_url`. The daemon polls it over the node's mTLS connection every `compute.health_interval_seconds`; the endpoint reports `status`, `available_slots`, and `loaded_models`. A remote node is ineligible for placement until a successful probe confirms available capacity and the registered model. `/control` shows the last probe and capacity report, and operators can trigger a probe directly.
