...
connectd init --admin-name 'System owner'
connectd serve
```

Set `daemon.signing_key_path` to the key path. `connectd init` upgrades the database, creates a named `daemon_admin`, and prints its one-time token; store it securely. Genesis expires the bootstrap token for API access. The daemon still reads `CONNECTD_OPERATOR_TOKEN` at startup so it can recognize and reject the expired credential. The API binds to loopback by default. Visit `http://127.0.0.1:8790/control` and enter the daemon admin token. The organization setup panel creates an organization and issues one-time local user tokens with `admin`, `operator`, or `viewer` roles. Organization admins can issue and revoke users in their own organization. Organization members see only their tasks, steps, budgets, and audit timelines; operators can add steps directly from the task workspace. Set `CONNECTD_CLIENT_TOKEN` to an organization operator token when running `connectd worker run-step`. Workers read memory from their task's organization. Organization admins can register tools and compute nodes owned by their organization; the daemon admin can manage genesis and global memory but cannot override organization registry shares or RunPod fallback. Registry discovery, worker routing, model placement, and grant redemption check organization ownership or an explicit share. Existing and migrated entries belong to the Default organization; other organizations need a share before use. An organization admin can grant and revoke individual tool or node shares, or share an entire tool or node registry using `*`, through `/api/v1/orgs/{org_id}/shares` and `/control`. The API also supports direct clients with `Authorization: Bearer <token>`.
...
The provider-neutral `ProvisioningAdapter` contract has a RunPod REST v2 reference implementation for Pod create, read, list, and delete operations. Its read-only quote method fetches the RunPod GPU catalog list price and availability for the requested cloud and GPU count; it excludes storage and is not a binding Pod rate. Its API key is resolved at call time from `RUNPOD_API_KEY` or an explicitly configured local env file such as `secrets/connectd.env`; the `secrets/` directory is ignored by Git. The bounded provisioner requires that quote by default, rejects a quote over the operator's hourly cap before creation, and deletes a newly created Pod if its reported rate exceeds the cap. An organization admin can preview the GPU catalog price through `/api/v1/orgs/{org_id}/provisioning/runpod/quote` or the Control workspace and can turn on the unquoted fallback for their organization through `/api/v1/orgs/{org_id}/settings`. Set `provisioning.runpod_secret_file` to use a local secret file instead of an environment variable. The fallback remains off by default. Automatic paid RunPod creation is disabled in V1. `provisioning.enable_paid_runpod_leases` defaults to `false`, and startup rejects `true` while REST v2 lacks a verified provider termination deadline. RunPod removed its former timer after finding that it did not stop billing. The reference adapter supports quoted Pod operations, but a process-level watchdog is not a billing cutoff. Lease budgeting and provider-neutral lifecycle admission remain in development. Provisioned Pods do not become trusted inference nodes automatically: operator-owned node registry metadata and mTLS health admission still apply. RunPod Pod billing history is available as a read-only, Pod-specific provider charge lookup for future lease settlement. Automatic lifecycle dispatch remains gated on the operator-defined lease and storage budget policy.
...
Memory recall retains confidence, validity dates, tags, source provenance, supersession, and contradiction warnings. Each organization sets `memory_authority` (`session_auto`, `hybrid`, or `human_gated`), defaulting to `hybrid`; a task's execution profile can only make it stricter. Workers capture into durable `task:<task_id>` memory. `session_auto` and `hybrid` trust task claims during the active task, while `human_gated` keeps them pending until an organization admin promotes them. An organization admin can copy a reviewed task claim into permanent `org:<org_id>` memory through `/api/v1/memory/claims/{claim_id}/promote-to-org`, preserving the original task claim for audit. Promoted `repo:<id>` claims remain available across tasks bound to that repository. Worker context includes currently valid, trusted claims from its active task, repository, organization, and global scope. `worker_brief` projects a smaller payload. `/recall`, `/api/v1/memory/recall`, and the `/control` memory ledger keep stale, superseded, and pending claims visible to authorized organization readers for audit.

In `session_auto`, each task capture also creates a separate, untrusted organization candidate and a durable evaluation job. The daemon's librarian queue checks current policy, privacy class, source provenance, validity, and open contradictions. Only an independent evaluator response with `is_corroborated=true`, `contradiction_detected=false`, and score at or above `memory.evaluator.confidence_threshold` promotes the organization candidate. The worker's submitted confidence never drives this decision. The default evaluator is `fail_closed`, leaving jobs pending. To connect a trusted service, set `memory.evaluator.type: http_json` and `endpoint_url`; its endpoint accepts `claim_text`, `provenance_context` (JSON source metadata), and `existing_org_claims`, and returns `confidence_score`, `is_corroborated`, `contradiction_detected`, and `reasoning_summary`. A remote HTTPS evaluator requires both daemon `memory.evaluator.allow_remote_https: true` and the owning organization admin's opt-in through `/api/v1/orgs/{org_id}/remote-librarian` or `/control`. HTTP evaluation is limited to approved local hostnames. `allowed_privacy_classes` defaults to `public`; add sensitive classes only after approving that service's data handling. Unreachable or unapproved evaluators leave jobs pending for retry. Rejected assessments remain untrusted for organization recall and available for admin review.
