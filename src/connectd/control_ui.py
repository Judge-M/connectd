...
<section class="card"><h2>Memory ledger</h2><div class="row"><div><label for="memory-scope">Scope</label><input id="memory-scope" placeholder="repo:my-project"></div><div><button id="load-memory">Inspect claims</button></div></div><div id="memory-ledger" class="muted">Enter a scope to inspect promoted, stale, superseded, and pending claims.</div></section>
<section class="card"><h2>Compute nodes</h2><div id="compute-nodes" class="muted">Load workspace to inspect node health and capacity.</div></section>
<script>
...
async function load(){token=byId('token').value.trim();if(!token){status('Enter an operator token.',true);return}try{const [tasks,candidates,nodes]=await Promise.all([api('/api/v1/tasks'),api('/api/v1/memory/candidates'),api('/api/v1/compute/nodes')]);renderTasks(tasks);renderCandidates(candidates);renderNodes(nodes);status('Workspace loaded.')}catch(e){status(e.message,true)}}
